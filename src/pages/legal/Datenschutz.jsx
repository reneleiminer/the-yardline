...
export default function Datenschutz() {
  useSetHeader({ mode: 'back', title: 'Datenschutz' });

  const { data: page, isLoading } = useQuery({
    queryKey: ['legalPage', 'datenschutz'],
    queryFn: async () => {
      const pages = await base44.entities.LegalPage.filter({ slug: 'datenschutz' });
      return pages[0];
    },
  });
    ...
    return (
      <div className="w-full max-w-3xl mx-auto px-4 py-8 pb-24">
        <div className="rounded-2xl border border-border/50 bg-card px-4 py-8 text-center">
          <h1 className="text-xl font-black mb-2">Datenschutz</h1>
  ...
  return <LegalPageTemplate title={page.title || 'Datenschutz'} content={page.content} />;
}