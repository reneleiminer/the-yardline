...
Deno.serve(async (req) => {
  try {
    ...
    // Check for existing share notification to avoid duplicates
    ...
    const notification = await base44.entities.Notification.create({
      userId: post.authorId,
      type: 'post_share',
      title: `${actor.username || 'Jemand'} hat deinen Beitrag geteilt`,
      message: `${actor.username || 'Jemand'} hat deinen Beitrag geteilt`,
      targetType: 'post',
      targetId: postId,
