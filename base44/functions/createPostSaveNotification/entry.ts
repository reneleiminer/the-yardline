...
Deno.serve(async (req) => {
  try {
    ...
    // Check for existing save notification to avoid duplicates
    ...
    // Get actor info
    const actor = await base44.entities.AppUser.get(user.id);

    // Create notification
    const notification = await base44.entities.Notification.create({
      userId: post.authorId,
      type: 'post_save',
      title: `${actor.username || 'Jemand'} hat deinen Beitrag gespeichert`,
      message: `${actor.username || 'Jemand'} hat deinen Beitrag gespeichert`,
      targetType: 'post',
      targetId: postId,
